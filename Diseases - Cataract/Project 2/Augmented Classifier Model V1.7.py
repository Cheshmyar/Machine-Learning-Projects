...
log_dir = "./Logs/Augmented Classifier Model V1.7 - " + str(int(ratio * 100)) + "%/"
...
cls.fit(it_train, steps_per_epoch=int(inputs.shape[0]/BATCH_SIZE), shuffle=True, epochs=150, validation_data=it_valid,
        validation_steps=int(inputs_valid.shape[0]/BATCH_SIZE), callbacks=[tensorboard_callback, csv_logger, reduce_lr])

cls.save("./Models/Augmented Classifier Model V1.7 - " + str(int(ratio * 100)) + "%/")
...
print(predicts.shape)
acc = np.mean(outputs_valid == predicts)
